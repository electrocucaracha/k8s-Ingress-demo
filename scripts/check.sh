...
assert_contains "$(curl -s "http://$(kubectl get ingress website-ingress -o jsonpath='{.status.loadBalancer.ingress[0].ip}')/en")" "Hello"
assert_contains "$(curl -s "http://$(kubectl get ingress website-ingress -o jsonpath='{.status.loadBalancer.ingress[0].ip}')/es")" "Hola"
